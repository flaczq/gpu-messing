...
void ResourceManager::uploadToGPU(unsigned char* data, unsigned int& textureID, int width, int height, int channels, bool isBGR) {
	...
	// filter params
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, gpuFormat == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, gpuFormat == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT);
